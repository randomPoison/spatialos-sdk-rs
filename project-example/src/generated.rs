...
impl TypeConversion for Rotate {
    fn from_type(input: &SchemaObject) -> Result<Self, String> {
        Ok(Self {
            ...
            radius: input.field::<SchemaDouble>(5).get_or_default(),
            center_x: input.field::<SchemaDouble>(2).get_or_default(),
            center_y: input.field::<SchemaDouble>(3).get_or_default(),
            center_z: input.field::<SchemaDouble>(4).get_or_default(),
        })
    }
    fn to_type(input: &Self, output: &mut SchemaObject) -> Result<(), String> {
        output.field::<SchemaDouble>(1).add(input.angle);
        output.field::<SchemaDouble>(5).add(input.radius);
        output.field::<SchemaDouble>(2).add(input.center_x);
        output.field::<SchemaDouble>(3).add(input.center_y);
        output.field::<SchemaDouble>(4).add(input.center_z);
        Ok(())
    }
}
impl ComponentData<Rotate> for Rotate {
    fn merge(&mut self, update: RotateUpdate) {
        ...
        if let Some(value) = update.center_x { self.center_x = value; }
        if let Some(value) = update.center_y { self.center_y = value; }
        if let Some(value) = update.center_z { self.center_z = value; }
    }
}
...
impl TypeConversion for RotateUpdate {
    fn from_type(input: &SchemaObject) -> Result<Self, String> {
        ...
        let _field_radius = input.field::<SchemaDouble>(5);
        if _field_radius.count() > 0 {
            let field = &_field_radius;
            output.radius = Some(field.get_or_default());
        }
        let _field_center_x = input.field::<SchemaDouble>(2);
        if _field_center_x.count() > 0 {
            let field = &_field_center_x;
            output.center_x = Some(field.get_or_default());
        }
        let _field_center_y = input.field::<SchemaDouble>(3);
        if _field_center_y.count() > 0 {
            let field = &_field_center_y;
            output.center_y = Some(field.get_or_default());
        }
        let _field_center_z = input.field::<SchemaDouble>(4);
        if _field_center_z.count() > 0 {
            let field = &_field_center_z;
            output.center_z = Some(field.get_or_default());
        }
        Ok(output)
    }
    fn to_type(input: &Self, output: &mut SchemaObject) -> Result<(), String> {
        ...
        if let Some(value) = input.radius {
            output.field::<SchemaDouble>(5).add(value);
        }
        if let Some(value) = input.center_x {
            output.field::<SchemaDouble>(2).add(value);
        }
        if let Some(value) = input.center_y {
            output.field::<SchemaDouble>(3).add(value);
        }
        if let Some(value) = input.center_z {
            output.field::<SchemaDouble>(4).add(value);
        }
        Ok(())
    }
}
impl ComponentUpdate<Rotate> for RotateUpdate {
    fn merge(&mut self, update: RotateUpdate) {
        ...
        if update.center_x.is_some() { self.center_x = update.center_x; }
        if update.center_y.is_some() { self.center_y = update.center_y; }
        if update.center_z.is_some() { self.center_z = update.center_z; }
    }
}
